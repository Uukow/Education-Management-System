...
function register_user($conn){
    ...
    $query = "CALL sp_user('','$name','$username',MD5('$password'),'$role','$statuss','$email','$verify')";
...
function update_user($conn){
    ...
    // Building the query and calling the stored procedure
    $query = "CALL sp_user('$id','$name','$username',MD5('$password'),'$role','$statuss','$email','$verify')";
...
function get_user($conn) {
    $data = array();
    $array_data = array();
    $query = "SELECT * FROM user WHERE `Username` != 'uukow';";
    $result = $conn->query($query);
